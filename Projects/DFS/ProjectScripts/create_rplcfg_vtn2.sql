      ...
      where r2.stoloc = lm.stoloc
        ...
        and (exists(select 'x'
                     ...
                     where i.stoloc = r2.stoloc
                       and i.prtnum <> r2.prtnum) or (lm.locsts = 'E' and exists(select 'x'
                                                                                   from inventory_view i,
                                                                                        locmst l2
...
if (@create_flg = 1)
{
    if (@asgare != '')
    {
        [select lmnew.stoloc stoloc_to_crt,
           ...
           left outer
           join tmp_loctype ltnew
             on lmnew.stoloc = ltnew.stoloc
           left outer
           join tmp_loctype ltold
          ...
          order by lmnew.asgflg,
                lmnew.stoloc] catch(-1403) >> res
        |
        if (@? = -1403)
        {
            [select lmnew.stoloc stoloc_to_crt,
        ...
        {
            [insert
               into tmp_rplcfg_comment(prtnum, prtfam, stoloc, arecod, ActionType, lngdsc)
             select @prtnum,
                    prtfam,
                    @asgloc,
                    @asgare,
    ...
    else
    {
        ...
        [select lm.stoloc stoloc_to_crt,
          ...
          where iv.prtnum = pv.prtnum
            ...
            and decode(lm.arecod, 'VLQRD', 'L', 'VLQRDC', 'L', 'VWIND', 'W', 'VWINDC', 'W', 'XXX') = decode(pv.dept_cod, '10', 'L', '11', 'W', '22', 'T', 'YYY')
        ...
        else
        {
            ...
            if (@? = 0)
            {
                [select 'x'
                   from dual
                  where decode(@inv_arecod, 'VLQRD', 'L', 'VLQRDC', 'L', 'VWIND', 'W', 'VWINDC', 'W', 'XXX') <> decode(@inv_dept_cod, '10', 'L', '11', 'W', '22', 'T', 'YYY')]
                |
                [insert
                   into tmp_rplcfg_comment(prtnum, prtfam, stoloc, arecod, ActionType, lngdsc)
                 select @prtnum,
            ...
            else
            {
                ...
                if (@? = 0)
                {
                    ...
                    |
                    [select decode(nvl(@prt_velzon, 'B'), '1', decode(@inv_dept_cod, '10', 'VLQRD', '11', 'VWIND'), '2', decode(@inv_dept_cod, '10', 'VLQRD', '11', 'VWIND'), '3', decode(@inv_dept_cod, '10', 'VLQRDC', '11', 'VWINDC'), '4', decode(@inv_dept_cod, '10', 'VLQRDC', '11', 'VWINDC'), 'XXX') dstare
                       from dual]
                    |
