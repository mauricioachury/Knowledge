       ...
       where r.prtnum = pv.prtnum
         ...
         and (exists(select 'z'
                      ...
                      where iv2.prtnum <> r.prtnum
                        ...
                        and iv2.stoloc = r.stoloc) or (l.locsts = 'E' and exists(select 'x'
                                                                                   from inventory_view i,
                                                                                        locmst lm
                                                                                  where i.stoloc = lm.stoloc
                                                                                    and i.prtnum = pv.prtnum
                                                                                    and lm.arecod = r.arecod
                                                                                    and lm.stoloc <> r.stoloc))))
...
[select r.stoloc asgloc,
  ...
  where r.stoloc = lm.stoloc
    ...
    and r.prtnum = @prtnum
    and r.arecod in ('VLQRD', 'VLQRDC', 'VWIND', 'VWINDC')] catch(-1403)
|
if (@? = 0)
{
    [select *
...
else
{
    [select stoloc asgloc
      ...
      where r.prtnum = @prtnum
        and r.arecod like 'V%'] catch(-1403)
    |
    if (@? = 0)
    {
        publish data
    ...
    if (@asgare != '')
    {
        [select lmnew.stoloc stoloc_to_crt,
          ...
          where iv.stoloc = lmnew.stoloc
            ...
            and iv.inv_attr_str1 = '1506'
                and nvl(decode(ltnew.velzon, '3', '3', '4', '3'), 'X') = nvl(decode(ltold.velzon, '3', '3', '4', '3'), 'X')
    ...
    else
    {
        ...
        [select lm.stoloc stoloc_to_crt,
          ...
          where iv.prtnum = pv.prtnum
            ...
            and decode(lm.arecod, 'VLQRD', 'L', 'VLQRDC', 'L',
                                  'VWIND', 'W', 'VWINDC', 'W', 'XXX') = decode(pv.dept_cod, '10','L','11', 'W', '22', 'T', 'YYY')
            and iv.stoloc = lm.stoloc
            and lm.arecod in ('VLQRD', 'VLQRDC', 'VWIND', 'VWINDC')
            and iv.prtnum = @prtnum
            and iv.inv_attr_str1 = '1506'
         ...
         else
         {
             ...
             {
                 [select 'x' from dual
                    where decode(@inv_arecod, 'VLQRD', 'L', 'VLQRDC', 'L',
                                              'VWIND', 'W', 'VWINDC', 'W', 'XXX') <> decode(@inv_dept_cod, '10','L','11', 'W', '22', 'T', 'YYY')
                 ]
                 |
                 [insert into tmp_rplcfg_comment(prtnum, prtfam, stoloc, arecod, ActionType, lngdsc )
                 select @prtnum,
             ...
             else
             {
                 [select lm.stoloc inv_stoloc,
                  ...
                  if (@? = 0)
                  {
                      ...
                      |
                      [select decode(nvl(@prt_velzon,'B'),  '1', decode(@inv_dept_cod, '10', 'VLQRD', '11', 'VWIND'),
                                                            '2', decode(@inv_dept_cod, '10', 'VLQRD', '11', 'VWIND'),
                                                            '3', decode(@inv_dept_cod, '10', 'VLQRDC','11', 'VWINDC'),
                                                            '4', decode(@inv_dept_cod, '10', 'VLQRDC','11', 'VWINDC'), 'XXX') dstare
                       from dual]
                       |
