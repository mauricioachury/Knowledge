[delete
   from tmp_rplcfg_comment
  where 1 = 1] catch(-1403)
|
[delete
   from tmp_rplcfg
  where 1 = 1] catch(-1403)
    ...
    else
    {
        ...
        [select lm.stoloc stoloc_to_crt,
          ...
          where iv.prtnum = pv.prtnum
            ...
            and decode(lm.arecod, 'LQRD', 'L', 'LQRDC', 'L', 'WIND', 'W', 'WINDC', 'W', 'TOBD', 'T', 'TOBDC', 'T', 'XXX') = decode(pv.dept_cod, '10', 'L', '11', 'W', '22', 'T', 'YYY')
        ...
        else
        {
            ...
            if (@? = 0)
            {
                [select 'x'
                   from dual
                  where decode(@inv_arecod, 'LQRD', 'L', 'LQRDC', 'L', 'WIND', 'W', 'WINDC', 'W', 'TOBD', 'T', 'TOBDC', 'T', 'XXX') <> decode(@inv_dept_cod, '10', 'L', '11', 'W', '22', 'T', 'YYY')]
                |
                [insert
                   into tmp_rplcfg_comment(prtnum, prtfam, stoloc, arecod, ActionType, lngdsc)
                 select @prtnum,
            ...
            else
            {
                ...
                if (@? = 0)
                {
                    ...
                    |
                    [select decode(nvl(@prt_velzon, 'B'), '1', decode(@inv_dept_cod, '10', 'LQRD', '11', 'WIND', '22', 'TOBD'), '2', decode(@inv_dept_cod, '10', 'LQRD', '11', 'WIND', '22', 'TOBD'), '3', decode(@inv_dept_cod, '10', 'LQRDC', '11', 'WINDC', '22', 'TOBDC'), '4', decode(@inv_dept_cod, '10', 'LQRDC', '11', 'WINDC', '22', 'TOBDC'), 'XXX') dstare
                       from dual]
                    |
